...
class UserAuth {
  ...
  public async login({ username, password }) {
    ...
    if (!isPasswordMatching)
      throw new HttpException(403, "wrong password, enter correct password. ");

    const apikey = genAPIKey(15);
    const updateUserKey = await USER.findByIdAndUpdate(findUser._id, {
      API_KEY: apikey,
    });
    const updateKeyUseage = await USER.findByIdAndUpdate(findUser._id, {
      max_key_useage: 300,
    });
